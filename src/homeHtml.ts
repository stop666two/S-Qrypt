...
<input type="password" id="init-setup-token" placeholder="部署时配置的 SETUP_TOKEN（初始化密钥）" autocomplete="new-password" spellcheck="false">
...
<button id="btn-backup">备份</button>
...
<button id="audit-clear">清除本地日志</button>
<span id="audit-key-status" style="font-size:11px;color:var(--text3);flex:1;text-align:right;line-height:2"></span>
<button id="audit-key-config" style="font-size:12px;padding:5px 12px;border-radius:6px;cursor:pointer;background:var(--surface2);border:1px solid var(--border);color:var(--text2)">配置审计密钥</button>
</div>
</div>
</div>
<div id="audit-key-overlay" style="position:fixed;inset:0;z-index:800;background:rgba(0,0,0,.75);display:none;align-items:center;justify-content:center;padding:20px;backdrop-filter:blur(3px)">
<div style="background:var(--surface);border:1px solid var(--border);border-radius:14px;width:560px;max-width:100%;max-height:90dvh;display:flex;flex-direction:column;box-shadow:0 16px 48px rgba(0,0,0,.4)">
...
<h3 style="font-size:16px;font-weight:600">审计日志公钥配置</h3>
<button id="audit-key-close" style="background:transparent;border:none;color:var(--text2);cursor:pointer;font-size:18px;padding:4px 8px;border-radius:6px">✕</button>
</div>
<div style="padding:20px 24px;overflow-y:auto;flex:1">
<p style="font-size:13px;color:var(--text2);margin-bottom:12px">粘贴 RSA 公钥 PEM（2048/4096 位），用于加密审计日志。</p>
<p id="audit-key-warning" style="font-size:12px;color:var(--warning);margin-bottom:12px;display:none"></p>
<textarea id="audit-key-textarea" placeholder="-----BEGIN PUBLIC KEY-----&#10;...&#10;-----END PUBLIC KEY-----" style="width:100%;height:160px;padding:12px;background:var(--surface2);border:1px solid var(--border);border-radius:8px;color:var(--text);outline:none;font-size:11px;font-family:monospace;resize:vertical;line-height:1.5"></textarea>
</div>
<div style="display:flex;align-items:center;justify-content:flex-end;gap:10px;padding:14px 24px;border-top:1px solid var(--border)">
<span id="audit-key-msg" style="font-size:12px;color:var(--text3);flex:1"></span>
<button id="audit-key-cancel" style="padding:8px 18px;border-radius:6px;cursor:pointer;font-size:13px;background:transparent;border:1px solid var(--border);color:var(--text2)">取消</button>
<button id="audit-key-save" style="padding:8px 18px;border-radius:6px;cursor:pointer;font-size:13px;background:var(--accent);border:none;color:#fff;font-weight:500">保存密钥</button>
</div>
</div>
</div>
...
function setHTML(el,html){if(el)el.innerHTML=ttPolicy?ttPolicy.createHTML(html):html}
...
function s256P(d){const ml=d.length*8,b=Math.ceil((d.length+9)/64),buf=new Uint8Array(b*64);buf.set(d);buf[d.length]=0x80;const dv=new DataView(buf.buffer);dv.setUint32(buf.length-8,Math.floor(ml/4294967296),0);dv.setUint32(buf.length-4,ml&0xffffffff,0);return buf}
...
async function submitAuditLog(type, detail, pubKey) {
  ...
  try {
    if (pubKey) {
      ...
      const fph = fpHash(fp);
      await apiF('/api/audit/log', { method: 'POST', body: JSON.stringify({ encrypted_entry: encrypted, fingerprint_hash: fph }), auth: true });
    }
...
try{const stored=localStorage.getItem('sq_audit');if(stored){const parsed=JSON.parse(stored);if(Array.isArray(parsed)){for(const e of parsed){if(!e||typeof e.ts!=='string')continue;if(!e.hash){const es=e.ts+(e.type||'')+(e.detail||'')+(e.sig||'');e.hash=hex(sha256(new TextEncoder().encode(es)))}auditLog.push(e);auditPrevHash=e.hash}}}
}catch(e){console.warn('audit hydration failed',e)}
async function addAE(type,detail){const chainStr=auditPrevHash+type+detail;let sig='unsigned';if(appState&&appState.mk){try{const ak=await hkdfE(appState.mk,'audit-sign-key',64),sb=await hmac(ak,new TextEncoder().encode(chainStr));sig=hex(sb).slice(0,16)}catch(e){}}const fp=getDeviceFP();const entry={ts:new Date().toISOString(),type,detail,sig,prev:auditPrevHash.slice(0,8),ua:fp.ua,plat:fp.plat,screen:fp.screen,tz:fp.tz};auditLog.push(entry);const entryStr=entry.ts+entry.type+entry.detail+entry.sig;auditPrevHash=hex(sha256(new TextEncoder().encode(entryStr)));entry.hash=auditPrevHash;try{const stored=JSON.parse(localStorage.getItem('sq_audit')||'[]');stored.push(entry);if(stored.length>500)stored.splice(0,stored.length-500);localStorage.setItem('sq_audit',JSON.stringify(stored))}catch(e){}submitAuditLog(type,detail,auditPublicKey)}
// Anomaly detection
...
// Auto-lock
const LT=60000,LEV=['mousemove','keydown','scroll','touchstart','click','wheel','pointermove','visibilitychange'];
...
// Render
function compareByCreatedAt(a,b){if(a.damaged&&!b.damaged)return 1;if(!a.damaged&&b.damaged)return -1;if(!a.created_at)return 1;if(!b.created_at)return -1;return a.created_at<b.created_at?-1:a.created_at>b.created_at?1:0}
let searchQuery='';
function renderL(){const c=document.getElementById('note-list'),e=document.getElementById('empty-state'),sq=searchQuery.toLowerCase();let f=appState.notes.filter(n=>!n.damaged);if(sq)f=f.filter(n=>n.title.toLowerCase().includes(sq));if(f.length===0){setHTML(c,'');e.style.display='block';return}e.style.display='none';const ts=f.slice(0,appState.displayedCount);let h='';for(const n of ts){const d=n.damaged?'damaged':'',t=n.damaged?'[数据损坏]':(n.title||'(无标题)');h+='<div class="note-item '+d+'" data-id="'+n.id+'"><div class="note-title">'+esc(t)+'</div><div class="note-time">'+fmtTime(n.created_at)+(n.updated_at&&n.updated_at!==n.created_at?' · 修改 '+fmtTime(n.updated_at):'')+'</div>';if(!n.damaged)h+='<div class="note-actions"><button class="btn-edit" data-id="'+n.id+'" data-action="edit">编辑</button><button class="btn-delete" data-id="'+n.id+'" data-action="delete">删除</button></div>';h+='</div>'}if(appState.displayedCount<f.length&&!sq)h+='<div class="load-more"><button id="btn-load-more">加载更多 ('+(f.length-appState.displayedCount)+')</button></div>';setHTML(c,h);c.querySelectorAll('.note-item').forEach(el=>{el.addEventListener('click',e=>{if(e.target.closest('.note-actions'))return;openN(parseInt(el.dataset.id))})});c.querySelectorAll('[data-action="edit"]').forEach(el=>{el.addEventListener('click',e=>{e.stopPropagation();editN(parseInt(el.dataset.id))})});c.querySelectorAll('[data-action="delete"]').forEach(el=>{el.addEventListener('click',e=>{e.stopPropagation();delN(parseInt(el.dataset.id))})});const lm=document.getElementById('btn-load-more');if(lm)lm.addEventListener('click',async()=>{const more=(await apiF('/api/notes?offset='+appState.displayedCount+'&limit=20',{auth:true})).notes.filter(r=>r.is_test!==1&&r.deleted!==1);const results=await Promise.allSettled(more.map(async row=>{try{const keys=await getNK(row.id),mb=await decGCM(keys.KA,row.encrypted_meta_packet),meta=JSON.parse(new TextDecoder().decode(mb));return{id:row.id,title:meta.title||'',created_at:row.created_at||meta.created_at||'',updated_at:row.updated_at||meta.updated_at||'',damaged:false}}catch(e){return{id:row.id,title:'',created_at:'',updated_at:'',damaged:true}}}));for(const r of results){if(r.status==='fulfilled')appState.notes.push(r.value);else{const idx=results.indexOf(r);appState.notes.push({id:idx>=0?more[idx].id:0,title:'',created_at:'',updated_at:'',damaged:true})}}appState.notes.sort(compareByCreatedAt);appState.displayedCount=Math.min(appState.displayedCount+20,appState.notes.length);renderL()})}
// Note operations
...
async function saveN(){const title=document.getElementById('editor-note-title').value.trim(),body=document.getElementById('editor-note-body').value;if(!title){showT('请输入标题','error');return}const btn=document.getElementById('editor-save');btn.disabled=true;btn.textContent='保存中...';const eid=appState.editingId;pendingOps++;try{const now=new Date().toISOString();if(eid){const keys=await getNK(eid),note=appState.notes.find(n=>n.id===eid),meta={title,created_at:note?note.created_at:now,updated_at:now},mp=await encGCM(keys.KA,new TextEncoder().encode(JSON.stringify(meta))),bp=await encGCM(keys.KB,new TextEncoder().encode(body));await apiF('/api/note/'+eid,{method:'PUT',body:JSON.stringify({operation_token:hex(appState.opToken),encrypted_meta_packet:mp,encrypted_body:bp})});addAE('update','Note '+eid)}else{const r=await apiF('/api/note',{method:'POST',body:JSON.stringify({operation_token:hex(appState.opToken)})}),nid=r.id,KA=await deriveKA(appState.mk,nid),KB=await deriveKB(appState.mk,nid),KC=await deriveKC(appState.mk,nid);appState._keyCache['k_'+nid]={KA,KB,KC};const meta={title,created_at:now,updated_at:now},mp=await encGCM(KA,new TextEncoder().encode(JSON.stringify(meta))),bp=await encGCM(KB,new TextEncoder().encode(body));try{await apiF('/api/note/'+nid,{method:'PUT',body:JSON.stringify({operation_token:hex(appState.opToken),encrypted_meta_packet:mp,encrypted_body:bp})})}catch(putErr){await apiF('/api/note/'+nid,{method:'DELETE',body:JSON.stringify({operation_token:hex(appState.opToken)})}).catch(()=>{});delete appState._keyCache['k_'+nid];throw putErr}addAE('create','Note '+nid)}document.getElementById('editor-overlay').classList.remove('active');await refreshNL();showT('笔记已保存','success')}catch(e){showT('保存失败: '+(e.message||''),'error')}finally{pendingOps--;btn.disabled=false;btn.textContent=eid?'保存':'创建'}}
// Refresh note list (parallel)
let refreshMutex=false;
async function refreshNL(){if(refreshMutex)return;refreshMutex=true;const ls=document.getElementById('loading-state');try{ls&&ls.classList.add('show');const data=await apiF('/api/notes?offset=0&limit=50',{auth:true});const rows=data.notes.filter(r=>r.is_test!==1&&r.deleted!==1),results=await Promise.allSettled(rows.map(async row=>{const keys=await getNK(row.id),mb=await decGCM(keys.KA,row.encrypted_meta_packet),meta=JSON.parse(new TextDecoder().decode(mb));return{id:row.id,title:meta.title||'',created_at:row.created_at||meta.created_at||'',updated_at:row.updated_at||meta.updated_at||'',damaged:false}})),notes=[];for(const r of results){if(r.status==='fulfilled')notes.push(r.value);else notes.push({id:rows[notes.length].id,title:'',created_at:'',updated_at:'',damaged:true})}notes.sort(compareByCreatedAt);appState.notes=notes;appState.displayedCount=Math.min(20,notes.length);renderL()}catch(e){showT('加载失败: '+(e.message||''),'error')}finally{ls&&ls.classList.remove('show');refreshMutex=false}}
// Login
async function handleLogin(){const pw=document.getElementById('login-password').value,err=document.getElementById('login-error'),btn=document.getElementById('login-btn'),prog=document.getElementById('login-progress'),pi=document.getElementById('login-progress-inner'),pt=document.getElementById('login-progress-text'),fb=document.getElementById('lock-fail-badge');err.textContent='';if(!pw){err.textContent='请输入密码';return}btn.disabled=true;btn.textContent='验证中...';pendingOps++;try{prog.style.display='block';pt.textContent='获取验证信息...';let td;try{td=await apiF('/api/token')}catch(e){if(e.message==='not_initialized'){document.getElementById('lock-screen').classList.add('hidden');document.getElementById('init-screen').classList.remove('hidden');btn.disabled=false;btn.textContent='解锁';prog.style.display='none';document.getElementById('init-password').focus();return}throw e}let ph;if(td.salt){ph=hex(sha256(td.salt+hex(sha224(pw))))}else{ph=hex(sha224(pw))};appState.verificationToken=ph;try{await apiF('/api/notes?offset=0&limit=1',{auth:true})}catch(e){if(e.message==='unauthorized'){err.textContent='密码错误';failCount++;if(failCount>=FT){fb.textContent='⚠ 连续'+failCount+'次失败 · 安全锁定已加强';fb.style.display='inline-block'}addAE('login_fail','Attempt #'+failCount);btn.disabled=false;btn.textContent='解锁';prog.style.display='none';return}throw e}failCount=0;fb.style.display='none';if(td.audit_public_key){try{auditPublicKey=await importRsaPub(td.audit_public_key)}catch(e){console.warn('审计公钥恢复失败:',e)}}pt.textContent='正在派生主密钥...';const r1=await deriveMK(pw,function(p,m){pi.style.width=p+'%';pt.textContent=m},failCount>=FT);appState.mk=r1.mk;appState.kdfVersion=r1.kdfV;const ic=await apiF('/api/init-check');if(typeof ic.kdf_version==='number'&&ic.kdf_version!==r1.kdfV){throw new Error('此浏览器无法以保险箱的密钥派生强度解锁 (kdf_version 不匹配)')}pt.textContent='派生操作授权令牌...';appState.opToken=await deriveOp(r1.mk);pt.textContent='正在加载笔记...';await refreshNL();prog.style.display='none';document.getElementById('lock-screen').classList.add('hidden');document.getElementById('main-screen').classList.add('active');startLT();addAE('login','Successful')}catch(e){err.textContent='登录失败: '+(e.message||'');addAE('login_error',e.message||'');prog.style.display='none'}finally{pendingOps--;btn.disabled=false;btn.textContent='解锁'}}
// Init
async function handleInit(){const pw=document.getElementById('init-password').value,pw2=document.getElementById('init-password-confirm').value,err=document.getElementById('init-error'),btn=document.getElementById('init-btn'),prog=document.getElementById('init-progress'),pi=document.getElementById('init-progress-inner'),pt=document.getElementById('init-progress-text');err.textContent='';if(!pw){err.textContent='请输入密码';return}if(pw.length<10){err.textContent='密码至少10个字符';return}if(pw!==pw2){err.textContent='两次密码输入不一致';return}const setupToken=document.getElementById('init-setup-token').value.trim();if(!setupToken){err.textContent='请输入部署时配置的 SETUP_TOKEN（服务器初始化密钥）';return}btn.disabled=true;btn.textContent='创建中...';prog.style.display='block';pt.textContent='正在派生主密钥...';try{const r1=await deriveMK(pw,function(p,m){pi.style.width=p+'%';pt.textContent=m});const MK=r1.mk;const kv=r1.kdfV;pt.textContent='正在创建测试笔记...';const saltBytes=crypto.getRandomValues(new Uint8Array(16));const salt=hex(saltBytes);const ph=hex(sha256(salt+hex(sha224(pw)))),opT=await deriveOp(MK),opTH=await opTokenHash(MK);const pubPem=document.getElementById('init-public-key').value.trim();if(pubPem){try{auditPublicKey=await importRsaPub(pubPem)}catch(e){err.textContent='公钥格式错误: '+e.message;btn.disabled=false;btn.textContent='创建保险箱';return}}const pl=await apiF('/api/note',{method:'POST',body:JSON.stringify({operation_token:hex(opT),setup_token:setupToken})}),tni=pl.id,tMeta={title:'__S-QRYPT_TEST__',created_at:new Date().toISOString(),updated_at:new Date().toISOString()},tBody='S-Qrypt v1.0.0 初始化验证',tKA=await deriveKA(MK,tni),tKB=await deriveKB(MK,tni),tKC=await deriveKC(MK,tni),tMP=await encGCM(tKA,new TextEncoder().encode(JSON.stringify(tMeta))),tBP=await encGCM(tKB,new TextEncoder().encode(tBody));await apiF('/api/note/'+tni,{method:'PUT',body:JSON.stringify({operation_token:hex(opT),encrypted_meta_packet:tMP,encrypted_body:tBP,is_test:1,setup_token:setupToken})});await apiF('/api/init',{method:'POST',body:JSON.stringify({verification_token:ph,salt:salt,operation_token_hash:opTH,kdf_version:kv,audit_public_key:pubPem||'',setup_token:setupToken})});appState.verificationToken=ph;const vd=await apiF('/api/note/'+tni,{auth:true}),vmb=await decGCM(tKA,vd.encrypted_meta_packet),vm=JSON.parse(new TextDecoder().decode(vmb)),vbb=await decGCM(tKB,vd.encrypted_body),vb=new TextDecoder().decode(vbb);if(vm.title!=='__S-QRYPT_TEST__'||vb!==tBody)throw new Error('自检验证失败');appState.mk=MK;appState.opToken=opT;appState._keyCache['k_'+tni]={KA:tKA,KB:tKB,KC:tKC};await refreshNL();prog.style.display='none';document.getElementById('init-screen').classList.add('hidden');document.getElementById('main-screen').classList.add('active');startLT();showT('保险箱已就绪','success');addAE('init','Vault initialized')}catch(e){err.textContent='初始化失败: '+(e.message||'');prog.style.display='none'}finally{btn.disabled=false;btn.textContent='创建保险箱'}}
// Audit
function showLocalAudit(){const body=document.getElementById('audit-body');if(auditLog.length===0){setHTML(body,'<div class="audit-empty">暂无日志</div>');return}let h='<div style="font-size:11px;color:var(--text2);padding:4px 0 8px;border-bottom:1px solid var(--border)">链: '+esc(auditPrevHash.slice(0,16))+'… 共 '+auditLog.length+' 条</div>';const entries=[...auditLog].reverse();for(const e of entries){const sigTag=e.sig!=='unsigned'?' <span style="color:var(--success)">✓'+esc(e.sig.slice(0,6))+'</span>':'';const info=e.screen||e.ua||e.plat?' <span style="color:var(--text3);font-size:10px">'+esc(e.screen||'')+' '+(e.tz||'')+'</span>':'';h+='<div class="audit-entry"><span class="ts">['+e.ts.slice(11,19)+']</span> <span class="ev">'+esc(e.type)+'</span> '+esc(e.detail||'')+sigTag+info+'</div>'}setHTML(body,h)}
async function downloadRemoteAudit(){const btn=document.getElementById('audit-download');btn.disabled=true;btn.textContent='下载中...';try{const data=await apiF('/api/audit/logs?limit=200',{auth:true});if(!data.entries||data.entries.length===0){showT('暂无远程日志','error');return}const logs=data.entries.map(e=>({ts:e.created_at,encrypted:e.encrypted_entry}));const blob=new Blob([JSON.stringify({total:data.total,logs},null,2)],{type:'application/json'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download='s-qrypt-audit-logs.json';a.click();URL.revokeObjectURL(url);showT('已下载 '+logs.length+' 条加密日志','success')}catch(e){showT('下载失败: '+(e.message||''),'error')}finally{btn.disabled=false;btn.textContent='下载日志'}}
function updateAuditKeyStatus(){const el=document.getElementById('audit-key-status');if(auditPublicKey){el.innerHTML='<span style="color:var(--success)">● 审计密钥已配置</span>'}else{el.innerHTML='<span style="color:var(--text3)">○ 未配置审计密钥</span>'}}
function openAuditKeyConfig(){const overlay=document.getElementById('audit-key-overlay');const warn=document.getElementById('audit-key-warning');const ta=document.getElementById('audit-key-textarea');ta.value='';if(auditPublicKey){warn.style.display='block';warn.innerHTML='<b>⚠ 更换公钥将导致已有远程日志无法解密，旧日志将被自动删除。</b>'}else{warn.style.display='none'}document.getElementById('audit-key-msg').textContent='';overlay.style.display='flex';setTimeout(()=>ta.focus(),100)}
async function saveAuditKey(){const ta=document.getElementById('audit-key-textarea');const msg=document.getElementById('audit-key-msg');const pem=ta.value.trim();if(!pem){msg.textContent='请输入公钥 PEM';return}msg.textContent='';let key;try{key=await importRsaPub(pem)}catch(e){msg.textContent='公钥格式错误: '+e.message;return}const btn=document.getElementById('audit-key-save');btn.disabled=true;btn.textContent='保存中...';try{const hadKey=!!auditPublicKey;if(hadKey){await apiF('/api/audit/logs',{method:'DELETE',body:JSON.stringify({operation_token:hex(appState.opToken)})})}await apiF('/api/audit/key',{method:'PUT',body:JSON.stringify({operation_token:hex(appState.opToken),audit_public_key:pem})});auditPublicKey=key;updateAuditKeyStatus();document.getElementById('audit-key-overlay').style.display='none';showT(hadKey?'公钥已更新，旧日志已清除':'审计公钥已配置','success')}catch(e){msg.textContent='保存失败: '+(e.message||'')}finally{btn.disabled=false;btn.textContent='保存密钥'}}
// Init check
// Trash / Recycle bin
async function loadTrash(){const data=await apiF('/api/notes?deleted=1',{auth:true}),rows=data.notes;if(rows.length===0){setHTML(document.getElementById('trash-body'),'<div class="audit-empty">暂无已删除笔记</div>');return}let h='';for(const r of rows){let title='(未知)';try{const KA=await deriveKA(appState.mk,r.id),mb=await decGCM(KA,r.encrypted_meta_packet),meta=JSON.parse(new TextDecoder().decode(mb));title=meta.title||'(无标题)'}catch(e){title='(无法解密)'}h+='<div class="audit-entry">'+esc(title)+' <button class="btn-restore" data-id="'+r.id+'">恢复</button><button class="btn-purge" data-id="'+r.id+'">永久删除</button></div>'}setHTML(document.getElementById('trash-body'),h);document.querySelectorAll('#trash-body .btn-restore').forEach(el=>{el.addEventListener('click',async()=>{const id=parseInt(el.dataset.id);try{await apiF('/api/note/'+id+'/restore',{method:'PATCH',body:JSON.stringify({operation_token:hex(appState.opToken)})});showT('笔记已恢复','success');document.getElementById('trash-panel').classList.remove('active');await refreshNL()}catch(e){showT('恢复失败: '+(e.message||''),'error')}})});document.querySelectorAll('#trash-body .btn-purge').forEach(el=>{el.addEventListener('click',async()=>{const id=parseInt(el.dataset.id);const sure=await showC('永久删除不可恢复！确认？');if(!sure)return;try{await apiF('/api/note/'+id,{method:'DELETE',body:JSON.stringify({operation_token:hex(appState.opToken)})});showT('已永久删除','success');loadTrash();await refreshNL()}catch(e){showT('删除失败: '+(e.message||''),'error')}})})}

async function checkInit(){try{const data=await apiF('/api/init-check');if(data.db_bound===false){document.getElementById('lock-screen').classList.remove('hidden');document.getElementById('login-error').innerHTML='<span style="color:var(--warning)">⚠ D1 数据库未绑定</span><br><span style="font-size:12px">请在 Cloudflare Dashboard 中添加 D1 数据库绑定<br>或运行 <code>npm run setup</code> 自动配置</span>';document.getElementById('init-screen').classList.add('hidden');document.getElementById('login-btn').disabled=true;return}if(!data.initialized){document.getElementById('lock-screen').classList.add('hidden');document.getElementById('init-screen').classList.remove('hidden');document.getElementById('init-password').focus();return}appState.kdfVersion=data.kdf_version||1;document.getElementById('init-screen').classList.add('hidden');document.getElementById('lock-screen').classList.remove('hidden');document.getElementById('login-password').focus()}catch(e){document.getElementById('lock-screen').classList.remove('hidden');document.getElementById('login-error').textContent='无法连接服务器'}}
async function backupAll(){pendingOps++;const toast=document.getElementById('status-toast');toast.textContent='正在准备备份...';toast.className='status-toast show';try{let offset=0,allRows=[],total=Infinity;while(offset<total){const d=await apiF('/api/notes?offset='+offset+'&limit=500',{auth:true});const batch=(d.notes||[]).filter(r=>Number(r.is_test)!==1&&Number(r.deleted)!==1);allRows=allRows.concat(batch);total=d.total;offset+=d.notes.length;if(!d.notes||d.notes.length===0)break}const rows=allRows;let ok=0,fail=0;const results=[];for(let i=0;i<rows.length;i++){const row=rows[i];toast.textContent='正在备份 ('+(i+1)+'/'+rows.length+')...';try{const full=await apiF('/api/note/'+row.id,{auth:true});if(!full.encrypted_body){fail++;continue}const keys=await getNK(row.id),mb=await decGCM(keys.KA,full.encrypted_meta_packet),meta=JSON.parse(new TextDecoder().decode(mb)),bb=await decGCM(keys.KB,full.encrypted_body),bodyText=new TextDecoder().decode(bb);results.push({id:row.id,title:meta.title||'',body:bodyText,created_at:row.created_at||meta.created_at,updated_at:row.updated_at||meta.updated_at});ok++}catch(e){fail++;continue}}const blob=new Blob([JSON.stringify({exported_at:new Date().toISOString(),total:results.length,notes:results},null,2)],{type:'application/json;charset=utf-8'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='s-qrypt-backup-'+new Date().toISOString().slice(0,10)+'.json';a.click();URL.revokeObjectURL(url);toast.textContent='备份完成：成功 '+ok+' 篇'+(fail?'，'+fail+' 篇跳过':'')+(fail?'（已存入下载文件）':'');toast.className='status-toast show success';setTimeout(()=>toast.classList.remove('show'),5000)}catch(e){toast.textContent='备份失败: '+(e.message||'');toast.className='status-toast show error';setTimeout(()=>toast.classList.remove('show'),5000)}finally{pendingOps--}}
async function importBackup(){const inp=document.getElementById('import-file');if(!inp.files||!inp.files.length)return;const file=inp.files[0];inp.value='';if(!appState.mk){showT('请先解锁保险箱','error');return}const toast=document.getElementById('status-toast');toast.textContent='正在读取备份文件...';toast.className='status-toast show';try{let data;try{data=JSON.parse((await file.text()).replace(/^\uFEFF/,''))}catch(e){throw new Error('备份文件不是有效 JSON: '+(e.message||'解析失败'))}if(!data||!Array.isArray(data.notes))throw new Error('不是有效的 S-Qrypt 备份文件（缺少 notes 数组）');const total=data.notes.length;let ok=0,fail=0,errs=[];for(let i=0;i<total;i++){const n=data.notes[i];toast.textContent='正在导入 ('+(i+1)+'/'+total+')...';try{if(!n||typeof n.title!=='string'||typeof n.body!=='string')throw new Error('缺少 title/body');const created=await apiF('/api/note',{method:'POST',body:JSON.stringify({operation_token:hex(appState.opToken)})}),nid=created.id,KA=await deriveKA(appState.mk,nid),KB=await deriveKB(appState.mk,nid),KC=await deriveKC(appState.mk,nid);appState._keyCache['k_'+nid]={KA,KB,KC};const meta={title:n.title,created_at:(typeof n.created_at==='string'&&n.created_at)?n.created_at:new Date().toISOString(),updated_at:(typeof n.updated_at==='string'&&n.updated_at)?n.updated_at:new Date().toISOString()},mp=await encGCM(KA,new TextEncoder().encode(JSON.stringify(meta))),bp=await encGCM(KB,new TextEncoder().encode(n.body));try{await apiF('/api/note/'+nid,{method:'PUT',body:JSON.stringify({operation_token:hex(appState.opToken),encrypted_meta_packet:mp,encrypted_body:bp,created_at:meta.created_at,updated_at:meta.updated_at})})}catch(putErr){await apiF('/api/note/'+nid,{method:'DELETE',body:JSON.stringify({operation_token:hex(appState.opToken)})}).catch(()=>{});delete appState._keyCache['k_'+nid];throw putErr}ok++}catch(e){fail++;errs.push(String(n.title||n.id||('#'+(i+1)))+': '+(e.message||e))}}await refreshNL();toast.textContent='导入完成：成功 '+ok+' 篇'+(fail?'，失败 '+fail+' 篇：'+errs.slice(0,3).join('；'):'');toast.className='status-toast show '+(fail?'error':'success');setTimeout(()=>toast.classList.remove('show'),fail?8000:5000)}catch(e){toast.textContent='导入失败: '+(e.message||'');toast.className='status-toast show error';setTimeout(()=>toast.classList.remove('show'),8000)}}
// Events
document.addEventListener('DOMContentLoaded',()=>{checkInit();document.getElementById('login-btn').addEventListener('click',handleLogin);document.getElementById('login-password').addEventListener('keydown',e=>{if(e.key==='Enter')handleLogin()});document.getElementById('init-btn').addEventListener('click',handleInit);document.getElementById('init-password-confirm').addEventListener('keydown',e=>{if(e.key==='Enter')handleInit()});document.getElementById('btn-new').addEventListener('click',()=>{appState.editingId=null;document.getElementById('editor-title').textContent='新建笔记';document.getElementById('editor-note-title').value='';document.getElementById('editor-note-body').value='';document.getElementById('editor-overlay').classList.add('active');markEditorClean();document.getElementById('editor-note-title').oninput=markEditorDirty;document.getElementById('editor-note-body').oninput=markEditorDirty});document.getElementById('btn-backup').addEventListener('click',backupAll);document.getElementById('btn-import').addEventListener('click',()=>{document.getElementById('import-file').click()});document.getElementById('import-file').addEventListener('change',importBackup);document.getElementById('btn-lock').addEventListener('click',lockScr);let searchTimer;document.getElementById('search-input').addEventListener('input',function(){clearTimeout(searchTimer);searchTimer=setTimeout(()=>{searchQuery=this.value;appState.displayedCount=Math.min(20,appState.notes.length);renderL()},200)});document.getElementById('btn-trash').addEventListener('click',async()=>{await loadTrash();document.getElementById('trash-panel').classList.add('active')});document.getElementById('trash-close').addEventListener('click',()=>{document.getElementById('trash-panel').classList.remove('active')});document.getElementById('btn-audit').addEventListener('click',()=>{showLocalAudit();updateAuditKeyStatus();document.getElementById('audit-panel').classList.add('active')});document.getElementById('audit-close').addEventListener('click',()=>{document.getElementById('audit-panel').classList.remove('active')});document.getElementById('audit-clear').addEventListener('click',()=>{auditLog.length=0;localStorage.removeItem('sq_audit');auditPrevHash='';showLocalAudit();showT('日志已清除','success')});document.getElementById('audit-key-config').addEventListener('click',openAuditKeyConfig);document.getElementById('audit-key-close').addEventListener('click',()=>{document.getElementById('audit-key-overlay').style.display='none'});document.getElementById('audit-key-cancel').addEventListener('click',()=>{document.getElementById('audit-key-overlay').style.display='none'});document.getElementById('audit-key-save').addEventListener('click',saveAuditKey);document.getElementById('audit-local').addEventListener('click',()=>{showLocalAudit();document.getElementById('audit-download').style.display='none'});document.getElementById('audit-remote').addEventListener('click',()=>{if(auditPublicKey){setHTML(document.getElementById('audit-body'),'<div class="audit-empty">点击"下载日志"导出加密 JSON，用私钥离线解密</div>');document.getElementById('audit-download').style.display='inline-block'}else{setHTML(document.getElementById('audit-body'),'<div class="audit-empty">未配置 RSA 公钥，审计日志仅存储在本地浏览器</div>');document.getElementById('audit-download').style.display='none'}});document.getElementById('audit-download').addEventListener('click',downloadRemoteAudit);document.getElementById('detail-back').addEventListener('click',()=>{document.getElementById('detail-view').classList.remove('active');appState.currentNoteId=null});document.getElementById('detail-edit').addEventListener('click',()=>{if(appState.currentNoteId){document.getElementById('detail-view').classList.remove('active');editN(appState.currentNoteId)}});document.getElementById('editor-close').addEventListener('click',closeEditor);document.getElementById('editor-cancel').addEventListener('click',closeEditor);document.getElementById('editor-save').addEventListener('click',saveN);window.addEventListener('beforeunload',()=>{clearMem()});document.addEventListener('keydown',e=>{if(e.key==='Escape'){document.getElementById('detail-view').classList.remove('active');document.getElementById('audit-panel').classList.remove('active');document.getElementById('trash-panel').classList.remove('active');if(document.getElementById('editor-overlay').classList.contains('active')){closeEditor()}else{document.getElementById('editor-overlay').classList.remove('active')}}if((e.ctrlKey||e.metaKey)&&e.key==='n'){e.preventDefault();document.getElementById('btn-new').click()}if((e.ctrlKey||e.metaKey)&&e.key==='f'){e.preventDefault();document.getElementById('search-input').focus()}});if('serviceWorker'in navigator)navigator.serviceWorker.register('/sw.js').catch(()=>{});});
</script>
</body>
</html>`;
