...
function showLocalAudit(){const body=document.getElementById('audit-body');if(auditLog.length===0){setHTML(body,'<div class="audit-empty">暂无日志</div>');return}let h='<div style="font-size:11px;color:var(--text2);padding:4px 0 8px;border-bottom:1px solid var(--border)">链: '+esc(auditPrevHash.slice(0,16))+'…</div>';const entries=[...auditLog].reverse();for(const e of entries){const sigTag=e.sig!=='unsigned'?' <span style="color:var(--success)">✓'+esc(e.sig.slice(0,6))+'</span>':'';h+='<div class="audit-entry"><span class="ts">['+e.ts.slice(11,19)+']</span> <span class="ev">'+esc(e.type)+'</span> '+esc(e.detail||'')+sigTag+'</div>'}setHTML(body,h)}
