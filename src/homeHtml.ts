...
function frameCall(cmd,...args){return new Promise((r,j)=>{const id=++cryptoFrameId;const t=setTimeout(()=>{cryptoFramePending.delete(id);showT('沙箱响应超时，降级为纯软件模式','error');j(new Error('timeout'))},12000);cryptoFramePending.set(id,{resolve:v=>{clearTimeout(t);r(v)},reject:e=>{clearTimeout(t);j(e)}});cryptoFrame.contentWindow.postMessage({id,cmd,args},window.location.origin)})}
...
// Login
async function handleLogin(){const pw=document.getElementById('login-password').value,err=document.getElementById('login-error'),btn=document.getElementById('login-btn'),prog=document.getElementById('login-progress'),pi=document.getElementById('login-progress-inner'),pt=document.getElementById('login-progress-text'),fb=document.getElementById('lock-fail-badge');err.textContent='';if(!pw){err.textContent='请输入密码';return}btn.disabled=true;btn.textContent='验证中...';pendingOps++;try{prog.style.display='block';pt.textContent='获取验证信息...';let td;try{td=await apiF('/api/token')}catch(e){if(e.message==='not_initialized'){document.getElementById('lock-screen').classList.add('hidden');document.getElementById('init-screen').classList.remove('hidden');btn.disabled=false;btn.textContent='解锁';prog.style.display='none';document.getElementById('init-password').focus();return}throw e}const exp=td.verification_token;let ph;if(td.salt){ph=hex(sha256(td.salt+hex(sha224(pw))))}else{ph=hex(sha224(pw))};if(!ctEq(ph,exp)){err.textContent='密码错误';failCount++;if(failCount>=FT){fb.textContent='⚠ 连续'+failCount+'次失败 · 安全锁定已加强';fb.style.display='inline-block'}addAE('login_fail','Attempt #'+failCount);btn.disabled=false;btn.textContent='解锁';prog.style.display='none';return}failCount=0;fb.style.display='none';appState.verificationToken=ph;if(td.audit_public_key){try{auditPublicKey=await importRsaPub(td.audit_public_key)}catch(e){console.warn('审计公钥恢复失败:',e)}}pt.textContent='正在派生主密钥...';const r1=await deriveMK(pw,function(p,m){pi.style.width=p+'%';pt.textContent=m},failCount>=FT);appState.mk=r1.mk;appState.kdfVersion=r1.kdfV;pt.textContent='派生操作授权令牌...';appState.opToken=await deriveOp(r1.mk);pt.textContent='正在加载笔记...';await refreshNL();prog.style.display='none';document.getElementById('lock-screen').classList.add('hidden');document.getElementById('main-screen').classList.add('active');startLT();addAE('login','Successful')}catch(e){err.textContent='登录失败: '+(e.message||'');addAE('login_error',e.message||'');prog.style.display='none'}finally{pendingOps--;btn.disabled=false;btn.textContent='解锁'}}
// Init
async function handleInit(){const pw=document.getElementById('init-password').value,pw2=document.getElementById('init-password-confirm').value,err=document.getElementById('init-error'),btn=document.getElementById('init-btn'),prog=document.getElementById('init-progress'),pi=document.getElementById('init-progress-inner'),pt=document.getElementById('init-progress-text');err.textContent='';if(!pw){err.textContent='请输入密码';return}if(pw.length<4){err.textContent='密码至少4个字符';return}if(pw!==pw2){err.textContent='两次密码输入不一致';return}btn.disabled=true;btn.textContent='创建中...';prog.style.display='block';pt.textContent='正在派生主密钥...';try{const r1=await deriveMK(pw,function(p,m){pi.style.width=p+'%';pt.textContent=m});const MK=r1.mk;const kv=r1.kdfV;pt.textContent='正在创建测试笔记...';const saltBytes=crypto.getRandomValues(new Uint8Array(16));const salt=hex(saltBytes);const ph=hex(sha256(salt+hex(sha224(pw)))),opT=await deriveOp(MK),opTH=await opTokenHash(MK);const pubPem=document.getElementById('init-public-key').value.trim();if(pubPem){try{auditPublicKey=await importRsaPub(pubPem)}catch(e){err.textContent='公钥格式错误: '+e.message;btn.disabled=false;btn.textContent='创建保险箱';return}}const pl=await apiF('/api/note',{method:'POST',body:JSON.stringify({operation_token:hex(opT)})}),tni=pl.id,tMeta={title:'__S-QRYPT_TEST__',created_at:new Date().toISOString(),updated_at:new Date().toISOString()},tBody='S-Qrypt v1.0.0 初始化验证',tKA=await deriveKA(MK,tni),tKB=await deriveKB(MK,tni),tKC=await deriveKC(MK,tni),tMP=await encGCM(tKA,new TextEncoder().encode(JSON.stringify(tMeta))),tBP=await encGCM(tKB,new TextEncoder().encode(tBody));await apiF('/api/note/'+tni,{method:'PUT',body:JSON.stringify({operation_token:hex(opT),encrypted_meta_packet:tMP,encrypted_body:tBP,is_test:1})});await apiF('/api/init',{method:'POST',body:JSON.stringify({verification_token:ph,salt:salt,operation_token_hash:opTH,kdf_version:kv,audit_public_key:pubPem||''})});appState.verificationToken=ph;const vd=await apiF('/api/note/'+tni,{auth:true}),vmb=await decGCM(tKA,vd.encrypted_meta_packet),vm=JSON.parse(new TextDecoder().decode(vmb)),vbb=await decGCM(tKB,vd.encrypted_body),vb=new TextDecoder().decode(vbb);if(vm.title!=='__S-QRYPT_TEST__'||vb!==tBody)throw new Error('自检验证失败');appState.mk=MK;appState.opToken=opT;appState._keyCache['k_'+tni]={KA:tKA,KB:tKB,KC:tKC};await refreshNL();prog.style.display='none';document.getElementById('init-screen').classList.add('hidden');document.getElementById('main-screen').classList.add('active');startLT();showT('保险箱已就绪','success');addAE('init','Vault initialized')}catch(e){err.textContent='初始化失败: '+(e.message||'');prog.style.display='none'}finally{btn.disabled=false;btn.textContent='创建保险箱'}}
// Audit
